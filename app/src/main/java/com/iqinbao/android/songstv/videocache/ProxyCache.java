...
class ProxyCache {
    ...
    public void shutdown() {
        synchronized (stopLock) {
            MyLog.d(ProxyCacheUtils.LOG_TAG, "Shutdown proxy for " + source);
            try {
                stopped = true;
    ...
    protected final void onError(final Throwable e) {
        ...
        if (interruption) {
            MyLog.d(ProxyCacheUtils.LOG_TAG, "ProxyCache is interrupted");
        } else {
            MyLog.e(ProxyCacheUtils.LOG_TAG, "ProxyCache error", e);
        }
    }
